...
int main(int argc, char* argv[])
{
	...
	// NEW - RUN SEQUENCER
	if (argc < 3) {
		printf("Enter up to 3 short reads (all caps) as {2nd/3rd/4th} argument!\n");
		return -1;
	}
	...
	if (argc >= 3) {
		short_reads_to_run = 1;

		sr_bram_ptr[0] = get_sr_bram_ptr(0);
		
		int sr_size = 0;
		do {
			write_bram_value_char(sr_bram_ptr[0], sr_size, argv[2][sr_size]);
			sr_size++;
		} while (argv[2][sr_size] != 0);

		set_SR_Addr(sequencer_ptr[0], XPAR_AXI_BRAM_CTRL_SR_0_S_AXI_BASEADDR);
		set_SR_length(sequencer_ptr[0], sr_size);
	}
	if (argc >= 4) {
		short_reads_to_run = 2;

		sr_bram_ptr[1] = get_sr_bram_ptr(1);
		
		int sr_size = 0;
		do {
			write_bram_value_char(sr_bram_ptr[1], sr_size, argv[3][sr_size]);
			sr_size++;
		} while (argv[3][sr_size] != 0);

		set_SR_Addr(sequencer_ptr[1], XPAR_AXI_BRAM_CTRL_SR_1_S_AXI_BASEADDR);
		set_SR_length(sequencer_ptr[1], sr_size);
	}
	if (argc >= 5) {
		short_reads_to_run = 3;

		sr_bram_ptr[2] = get_sr_bram_ptr(2);
		
		int sr_size = 0;
		do {
			write_bram_value_char(sr_bram_ptr[2], sr_size, argv[4][sr_size]);
			sr_size++;
		} while (argv[4][sr_size] != 0);
